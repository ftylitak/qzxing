...
public:
    ...
    Ref<Result> constructResult(Pair leftPair, Pair rightPair);

    static boolean checkChecksum(Pair leftPair, Pair rightPair);

    Pair decodePair(Ref<BitArray> row, boolean right, int rowNumber, DecodeHints hints);

    DataCharacter decodeDataCharacter(Ref<BitArray> row, FinderPattern pattern, boolean outsideChar);
    ...
    void adjustOddEvenCounts(boolean outsideChar, int numModules);
