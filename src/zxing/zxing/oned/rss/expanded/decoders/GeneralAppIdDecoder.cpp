...
GeneralAppIdDecoder::GeneralAppIdDecoder(Ref<BitArray> information) : m_buffer("") {
...
DecodedNumeric GeneralAppIdDecoder::decodeNumeric(int pos)
{
    if (pos + 7 > m_information->getSize()) {
        ...
        if (numeric == 0) {
            return DecodedNumeric(m_information->getSize(), DecodedNumeric::FNC1, DecodedNumeric::FNC1);
        }
        return DecodedNumeric(m_information->getSize(), numeric - 1, DecodedNumeric::FNC1);
    }
...
{
    bool isFinished;
    BlockParsedResult result(false);
    do {
        ...
        boolean positionChanged = initialPosition != m_current.getPosition();
...
boolean GeneralAppIdDecoder::isAlphaTo646ToAlphaLatch(int pos)
{
...
boolean GeneralAppIdDecoder::isAlphaOr646ToNumericLatch(int pos)
{
...
boolean GeneralAppIdDecoder::isNumericToAlphaNumericLatch(int pos) {
